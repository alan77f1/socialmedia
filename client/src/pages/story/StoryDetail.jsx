...
function StoryDetail(props) {
  ...
  const [storyViewer, setStoryViewer] = useState(
    []
  ); /*story will show inside class right */
  ...
  const changeStoryIndexHandler = (number) => {
    if (
      showStoryIndex + number >= storyViewer.length ||
      showStoryIndex + number < 0
    ) {
      ...
      if (lastStory._id === storyViewer[showStoryIndex]._id && number === 1) {
        ...
        const currentIndex = allUser.findIndex(
          (user) => user._id === storyAuthor._id
        );
        ...
        const sViewer = stories.filter(
          (storyUser) =>
            storyUser[0].userId === allUser[currentIndex + number]._id
        );
