...
function StoryViewer(props) {
  ...
  const mouseMoveHandler = () => {
    if (pauseFlagBtn.current)
      return; /*cancel mouse event when click btn pause */
  ...
  const chooseLikeTypeHandler = (data) => {
    ...
    saveLikeTimeOut.current = setTimeout(() => {
      (async () => {
        await axios.put(
          `/stories/${storyViewer[showStoryIndex]._id}/likes`,
          saveReaction
        );
      })();
    }, 1500);
    ...
    >
      ...
      >
        ...
        <div className='storyItemContent'>
          {storyViewer[showStoryIndex]?.desc}
        </div>
      </div>
